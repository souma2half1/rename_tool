# 画像リネームツール

Tkinterを用いたGUIの画像リネームツールです。指定したフォルダ内の画像ファイルを
テーマ名に基づき`{テーマ名}_連番.拡張子`の形式で一括リネームします。

## バージョン 1.1

`オートリネーム`ボタンを追加し、選択したフォルダ以下のすべてのサブフォルダを再帰的に処理できるようになりました。各サブフォルダの名前をテーマ名として使用し、フォルダ内の画像を`<テーマ名>_連番.拡張子`形式でリネームします。

## 使い方

```
python rename_gui.py
```
1. 「参照」ボタンでフォルダを選択します。
2. テーマ名を入力します（空白は自動で`_`に変換されます）。
3. 「リネーム実行」ボタンを押すと、対応拡張子の画像がファイル名昇順で連番リネームされます。
4. 「オートリネーム」ボタンを押すと、選択したフォルダ以下のすべてのサブフォルダ内の画像を一括リネームします。

対応拡張子: `.png`, `.jpg`, `.jpeg`, `.webp`
